...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        try {
          ...
          console.error("AuthContext: Error fetching user role:", error.message || error);
          setRole(null);
        }
      } else {
        setUser(null);
  ...
  useEffect(() => {
    if (!loading && !user && !pathname.startsWith('/auth') && pathname !== '/') {
      // If not loading, no user, and not on an auth page or homepage, redirect to home
      // router.push('/'); // Example: redirect to home, signin might be more appropriate
    }
  ...
  if (loading && !pathname.startsWith('/auth')) { // Show loader only on non-auth pages during initial load
    return (
